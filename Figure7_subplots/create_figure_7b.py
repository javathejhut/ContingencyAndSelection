...
for sample in to_investigate:
    ...
    if labeled==False:
        ...
        plt.plot(fraction_range, frac_gc_content_data, 'k', linewidth=3,
...
fill_negative = plt.fill_between(np.arange(0.6, 1.1, 0.01), -0.1, 1. , color='gray', alpha= 0.3)
#plt.plot([0.6]*11, np.arange(0,1.1,0.1 ), 'b--', linewidth=3, alpha= 0.5, label='Window GC fraction that defines GC clusters')
plt.plot(fraction_range, ref_GC_fraction, 'r--', linewidth=3, label="Average GC content across mt reference")
...
plt.ylabel("Proportion of windows\nwith GC fraction g.t. x-axis", fontsize=size)
...
# #plt.show()
plt.savefig(path + "zero_ori_alternate_GC_coloured_by_freq.png", bbox_inches="tight", dpi=300)
#
# figure.clf()
