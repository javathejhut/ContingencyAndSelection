...
#
#         #if len(set) is multiple
#         longest_set = max(len(set(repeat_orientation)), len(set(repeat_lowhigh)))
...
#single transitions that have higher total span than primary junctions from transition merging are determined to be primary here
singlet_transitions = []
singlet_transitions_counts = []
primary_transition_lst = []

for i, trans in enumerate(primary_junctions_trans):
    to_add=True
    for test_trans in permute_only(trans):
        if test_trans in primary_transition_lst:
            to_add=False

    if to_add:
        primary_transition_lst.append(trans)

#print primary_junctions_trans, "start 1"

total_primary_junction_span = 0
for trans in primary_transition_lst:
    junc_1 = trans[0]
    junc_2 = trans[1]

    total_primary_junction_span+=abs(junction_ref_boundaries_dict[junc_1][0][2] - junction_ref_boundaries_dict[junc_2][1][2]) * sum(cleaned_trans_counts[0])/float(len(primary_transition_lst))

for row in range(transition_matrix.shape[0]):
    for col in range(transition_matrix.shape[1]):
        if row==col:
            singlet_junction = [key for key, value in junc_to_index_dict.items() if value == row][0]
            total_singlet_span = abs(junction_ref_boundaries_dict[singlet_junction][0][2]- junction_ref_boundaries_dict[singlet_junction][1][2]) * transition_matrix[row][col]

            #print row, total_singlet_span, total_primary_junction_span, transition_matrix[row][col]
            #if we can compare spans, take largest total monomer span to be primary
            if total_singlet_span > total_primary_junction_span and total_primary_junction_span > 0:
                singlet_transitions.append([[key for key, value in junc_to_index_dict.items() if value == row][0],[key for key, value in junc_to_index_dict.items() if value == col][0]])
                singlet_transitions_counts.append(transition_matrix[row][col])

            #if we never see anything else, consider singlet transition with largest span to be primary singlet
            elif total_singlet_span > total_primary_junction_span:

                singlet_junctions = []
                singlet_junction_counts = []
                for key in junction_counts_dict.keys():
                    singlet_junctions.append(key)
                    singlet_junction_counts.append(junction_counts_dict[key])

                singlet_junctions, singlet_junction_counts = (list(t) for t in zip(
                        *sorted(zip(singlet_junctions, singlet_junction_counts), key=lambda pair: pair[1],
                                reverse=True)))
                singlet_transitions.append([singlet_junctions[0], singlet_junctions[0]])
                singlet_transitions_counts.append(singlet_junction_counts[0])

#print singlet_transitions_counts, singlet_transitions
# test whether or not we already see this singlet from junction transition mering
# if we don't see intersection, treat singlets as true primary junctions
if len(singlet_transitions)!=0:
    singlet_transitions, singlet_transition_counts = (list(t) for t in  zip(*sorted(zip(singlet_transitions, singlet_transitions_counts), key=lambda pair: pair[1], reverse=True )))

    #if there is complete overlap between singlet and primary transitions, then we have already accounted for it, neglect it
    if (len(list(set(singlet_transitions[0]) & set(extract_ele_list_pairs(primary_junctions_trans))))== len(primary_junctions_trans)) and len(primary_junctions_trans)!=0:
        primary_junctions_trans = primary_junctions_trans

    #if there is no intersection at all, take this to be primary junction
    elif len(list(set(singlet_transitions[0]) & set(extract_ele_list_pairs(primary_junctions_trans)))) <= len(primary_junctions_trans) :
        primary_junctions_trans = singlet_transitions[0]

if not any(isinstance(el, list) for el in primary_junctions_trans):
    primary_junctions_trans = [primary_junctions_trans]

#print primary_junctions_trans, "start"

'''Parse all reads involved in primary alignments, recording how many sign transitions occur in each transition.
If in any junction transition the number of sign transitions != number of no sign transitions, consider this a "mixed" primary template'''
primary_trans_dict = {}

#initialize empty dict, storing [num no sign transitions, num sign transitions] for each abs transition in primary trans
for key in primary_junctions_trans:
    primary_trans_dict[tuple(key)] = [0,0]

for read_alphabet, read_orientation, read_low_high in zip(wholeref_read_alphabet, wholeref_read_alphabet_orientation, wholeref_read_alphabet_num_order):
    stripped_alphabet, stripped_orientation, stripped_lowhigh = strip_nontrue_junctions_allformats(read_alphabet, read_orientation, read_low_high, true_junctions)
    transitions_alphabet, transitions_orientation, transitions_lowhigh = return_junc_transitions_allformats(stripped_alphabet, stripped_orientation, stripped_lowhigh)

    # if len(read_alphabet) > 4:
    #     print read_alphabet
    for trans_alphabet, trans_ori, trans_lowhigh in zip(transitions_alphabet, transitions_orientation, transitions_lowhigh):

        #if we have a true transition and it is a "primary junction" transition
        trans_alphabet_absolute = [abs(ele) for ele in trans_alphabet]
        if len(trans_alphabet) ==2  and trans_alphabet_absolute in primary_junctions_trans:
            if sum(np.sign(trans_alphabet))==0:
                primary_trans_dict[tuple(trans_alphabet_absolute)][1] +=1
            else:
                primary_trans_dict[tuple(trans_alphabet_absolute)][0] += 1

#determine if this is a "mixed" repeat
primary_mixed_repeat=False
for key in primary_trans_dict.keys():
    #print primary_trans_dict[key]
    for trans in primary_trans_dict[key]:
        if max(primary_trans_dict[key]) !=0:
            if float(min(primary_trans_dict[key]))/max(primary_trans_dict[key]) >= 0.5:
                primary_mixed_repeat = True

print "is mixed?", primary_mixed_repeat
lcs_repeat_dict = return_lcs_repeats(wholeref_read_alphabet, wholeref_read_alphabet_orientation, wholeref_read_alphabet_num_order)

print lcs_repeat_dict
#populate example primary repeats from lcs repeat detection if they exist
primary_lcs_suggestion_dict = {}

for repeat in lcs_repeat_dict.keys():
    alphabet_version = [abs(ele) for ele in lcs_repeat_dict[repeat][-1]]
    if sorted(list(set(alphabet_version))) == sorted(list(set(primary_junctions_trans[0]))):
        if repeat not in primary_lcs_suggestion_dict.keys():
            primary_lcs_suggestion_dict[repeat] = lcs_repeat_dict[repeat][:]


print junction_ref_boundaries_dict

'''use junction reference boundaries to create an alignment space representation of all repeats from all sources'''

print primary_lcs_suggestion_dict, "testing lcs format here"

print lcs_repeat_dict, "start"

lcs_repeat_dict_ref = add_return_ref_locations_dict(junction_ref_boundaries_dict, lcs_repeat_dict)

print lcs_repeat_dict_ref, "end"

primary_lcs_suggestion_dict_ref = add_return_ref_locations_dict(junction_ref_boundaries_dict, primary_lcs_suggestion_dict)

mixed_alignment_count_dict =  return_mixed_alignment_count_dict(wholeref_read_alphabet, wholeref_read_alphabet_orientation, wholeref_read_alphabet_num_order, primary_junctions_trans)

#add longest stretch of junctions including primary junction transitions to exemplify mixed repeat

'''Computing molecular frequencies of detected repeats'''
#iterate through all detected repeat junctions, then record and sort them
true_junctions_in_repeats = []
for key in lcs_repeat_dict_ref.keys():
    for junc in key:
        true_junctions_in_repeats.append(int(junc[:-2]))

for lst in primary_junctions_trans:
    for ele in lst:
        true_junctions_in_repeats.append(int(ele))

true_junctions_in_repeats = sorted(list(set(true_junctions_in_repeats)))

#next construct a dictionary where keys represent binary vector representations of repeats in LCS or primary transition junctions
...
if primary_mixed_repeat:
    ...
    for lst in primary_junctions_trans:
        for junc in lst:
            from_primary.append(int(junc))

    from_primary = list(set(from_primary))

    for key in mixed_alignment_count_dict.keys():
        alignments.append(mixed_alignment_count_dict[key][0])

    primary_aln_key_count.extend([from_primary, alignments, [()], 0, 0])

else:
    from_primary = []
    alignments = []
    lcs_keys = []
    if len(primary_lcs_suggestion_dict_ref.keys())!=0:
        for key in primary_lcs_suggestion_dict_ref.keys():
            for junc in key:
                from_primary.append(int(junc[:-2]))
            for entry in primary_lcs_suggestion_dict_ref[key][-1]:
                alignments.append(entry)
            lcs_keys.append(key)
        from_primary=list(set(from_primary))

    # if we only have primary transitions to work with, remove those that are 0 to 85779 junctions, and keep the remainder
    else:
        example_primary_trans = primary_junctions_trans[0]
        print example_primary_trans, "example primary trans"
        to_remove_boundary_junc = []
        #handles case
        for junc in example_primary_trans:
            if  85679 < abs(junction_ref_boundaries_dict[junc][0][2] - junction_ref_boundaries_dict[junc][1][2]) < 85779:
                to_remove_boundary_junc.append(junc)
                print "here??"

        if len(to_remove_boundary_junc)==1:
            example_primary_trans.remove(to_remove_boundary_junc[0])

        #now construct inferred primary repeat from single junction present
        inferred_repeat_junction = example_primary_trans[0]

        alignments = [[junction_ref_boundaries_dict[inferred_repeat_junction][0][2],
                               junction_ref_boundaries_dict[inferred_repeat_junction][1][2],
                               junction_ref_boundaries_dict[inferred_repeat_junction][2][2],
                               junction_ref_boundaries_dict[inferred_repeat_junction][3][2]]]

        from_primary = [inferred_repeat_junction]

        lcs_keys = [(str(inferred_repeat_junction) + 'LH',)]

    primary_aln_key_count.extend([from_primary, alignments, lcs_keys, 0, 0])  # last is monomer count, second last molecule count

print primary_aln_key_count
print lcs_repeat_dict_ref, "here"
#now for all lcs vectors, making sure not to repeat primary vector representation (don't add if already in vector)
for key in lcs_repeat_dict_ref.keys():
    from_vec = []
    alignments = []
    lcs_keys = []
    ...
    for junc in key:
        from_vec.append(int(junc[:-2]))
    for entry in lcs_repeat_dict_ref[key][-1]:
        alignments.append(entry)

    lcs_keys.append(key)
    from_vec = list(set(from_vec))
    if not from_vec==primary_aln_key_count[0]:
        alternate_aln_key_count.append([from_vec, alignments, lcs_keys, 0, 0])

#construct primary alignments list (we are taking num +- 3*stdev here)
primary_alignments = []

for lst in primary_aln_key_count[1]:
    min_index = lst[:2].index(min(lst[:2]))
    max_index = 1 - min_index

    primary_alignments.append([int(lst[min_index] - 3 * lst[min_index + 2]), int(lst[max_index] + 3 * lst[max_index + 2])])

#construct alternate alignments list
alternate_alignments = []
for alternate in alternate_aln_key_count:
    for_single_alternate = []
    for lst in alternate[1]:
        min_index = lst[:2].index(min(lst[:2]))
        max_index = 1 - min_index

        for_single_alternate.append([int(lst[min_index] - 3 * lst[min_index + 2]), int(lst[max_index] + 3 * lst[max_index + 2])])
    alternate_alignments.append(for_single_alternate)

#before doing counting, eliminate alternate_aln_key_count entries if they aren't supported by inferred_binomial_repeats as long as set(junctions)>1
# to_remove = []
# for entry in alternate_aln_key_count:
#     per_repeat_juncs = entry[0]
#
#     if len(set(per_repeat_juncs))!=1:
#         binom_validated=False
#         for inferred in binom_inferred_repeats:
#             if sorted(list(set(per_repeat_juncs))) == sorted(list(set(inferred))):
#                 binom_validated=True
#         if not binom_validated:
#             to_remove.append(entry)
#
# for removed in to_remove:
#     alternate_aln_key_count.remove(removed)

#now do counting, both molecular and monomer
inferred_present=False
provide_example_mixed_read=False
if primary_mixed_repeat:
    example_mixed_read = {}
    provide_example_mixed_read=True

for read_alphabet, read_orientation, read_low_high, read_endpoints, read_juncformat in zip(wholeref_read_alphabet, wholeref_read_alphabet_orientation, wholeref_read_alphabet_num_order, wholeref_read_endpoints, returned_read_juncformat_wholeref):
...
contained_Petite_content = 0
contained_WT_content = 0

sample_name = output_name.strip().split('_')[0]

if len(inferred_primary) == 0:
    primary_junctions = primary_aln_key_count[0]

else:
    primary_junctions = inferred_primary[0]

#construct primary edges when asking the question if junctions exist inside primary structure for "conservative" calculations
sorted_primary_edges = [sorted([ele[0],ele[1]]) for ele in primary_aln_key_count[1]]

if sample_name == '12I1':
    sorted_primary_edges = [[sorted_primary_edges[1][0],
                                                       sorted_primary_edges[0][0]]]
if sample_name == "10I2" or sample_name == "11I2" or sample_name=="20I2" or sample_name=="21I2":
    #print sorted_primary_edges[sample_name]
    sorted_primary_edges = [[0, sorted_primary_edges[0][0]], [sorted_primary_edges[0][1], 85779]]

def consistent_with_primary(read_junction_format, primary_edges):
    junctions_in_read = [[ele[0], ele[1]] for ele in read_junction_format]

    count_not_consistent = 0
    for junc in junctions_in_read:
        first_inside_count = 0.0
        second_inside_count = 0.0
        for primary_edge in primary_edges:
            if junc[0] in range(primary_edge[0] - 2000, primary_edge[1] + 2000):
                first_inside_count += 1.0
            if junc[1] in range(primary_edge[0] - 2000, primary_edge[1] + 2000):
                second_inside_count += 1.0

        # print first_inside_count, second_inside_count, len()
        if first_inside_count == 0 or second_inside_count == 0:
            count_not_consistent+=1

    if count_not_consistent==0:
        return True
    else:
        return False
...
for read_alphabet, read_orientation, read_low_high, read_endpoints, read_juncformat, read_lengths in zip(wholeref_read_alphabet, wholeref_read_alphabet_orientation, wholeref_read_alphabet_num_order, wholeref_read_endpoints, returned_read_juncformat_wholeref, returned_read_lengths_wholeref):
    ...
    cleaned_of_artifacts = [j for j in read_alphabet if j!='-']
    cleaned_of_artifacts_abs = [abs(ele) if isinstance(ele, int) else ele for ele in cleaned_of_artifacts]
    ...
    elif len(set(cleaned_of_artifacts_abs)) != len(list(set(cleaned_of_artifacts_abs) & set(primary_junctions))):
    ...
    #     confident_WT_content+=read_lengths
    # #everything else goes to petite structure
    # elif len(set(cleaned_of_artifacts_abs)) != len(list(set(cleaned_of_artifacts_abs) & set(primary_junctions))) and '-' not in read_alphabet and '*' not in read_alphabet:
    ...
    # case of wt read but containing an artifact invdup
        ...
        confident_WT_content += read_lengths
    # everything else goes to petite structure
    elif len(set(cleaned_of_artifacts_abs)) != len(list(set(cleaned_of_artifacts_abs) & set(
            primary_junctions))) and '-' not in read_alphabet and '*' not in read_alphabet and consistent_with_primary(read_juncformat, sorted_primary_edges):
        confident_Petite_content += read_lengths
    else:
        confident_WT_content += read_lengths

    #do a version where we only consider non-wt to be real if contained within primary read, otherwise it is WT
    if len(read_alphabet) ==0:
        contained_WT_content+= read_lengths
        #print read_alphabet, read_endpoints, read_lengths
    #case of wt read but containing an artifact invdup
    elif len(read_alphabet) == 1 and '-' in read_alphabet:
        contained_WT_content+=read_lengths
    #everything else goes to petite structure
    elif len(set(cleaned_of_artifacts_abs)) != len(list(set(cleaned_of_artifacts_abs) & set(primary_junctions))) and consistent_with_primary(read_juncformat, sorted_primary_edges):
        contained_Petite_content+=read_lengths
    else:
        contained_WT_content+=read_lengths

    #print read_juncformat
    #print read_alphabet
    #print read_alphabet, cleaned_of_artifacts, read_endpoints, read_lengths
    #print set(cleaned_of_artifacts_abs), set(primary_junctions), len(set(cleaned_of_artifacts_abs)), len(list(set(cleaned_of_artifacts_abs) & set(primary_junctions)))
...
contained_petite_like_frac = float(contained_Petite_content)/(contained_Petite_content + contained_WT_content)

currDir = os.path.dirname(os.path.realpath(__file__))
folder_name = '/petite_fraction_petite/'
path = currDir + folder_name

if not os.path.exists(path):
    os.mkdir(path)

with open(path + "petites_Petite_like_fraction.txt", 'a') as f:
    f.write(output_name + "\t" + str(petite_like_frac) + "\t" + str(confident_petite_like_frac) + "\t" + str(contained_petite_like_frac) + "\t" + str(confident_Petite_content) + "\t" +str(confident_WT_content) +"\n")
